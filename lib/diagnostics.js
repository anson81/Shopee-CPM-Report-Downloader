...
(function (root, factory) {
  ...
  function buildReport(input) {
    ...
    // HOW OLD IS THE BACKGROUND WORKER.
    //
    // Chrome stops an idle MV3 worker after about 30 seconds and everything in
    // its memory goes with it. A worker younger than the run it is reporting on
    // restarted DURING that run - which means the filename listener was woken
    // by the very download it was being asked about, had nothing in memory, and
    // answered too late to matter. That is the leading explanation for files
    // landing loose while no other extension has touched them, and this line is
    // what turns it from a theory into a fact.
    if (i.workerStartedAt) {
      out.push(line('Background worker started', when(i.workerStartedAt, now) +
        ' (it is stopped after ~30s idle, and forgets everything)'));
    }
